#!/usr/bin/env python3
"""Run square H4 ground-state ADAPT-VQE at 1.5 Angstrom side length by default."""
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Square H4 ground-state ADAPT-VQE example (default side length: 1.5 A)."
    )
    parser.add_argument(
        "--side-length",
        type=float,
        default=1.5,
        help="H-H side length in Angstrom for square H4.",
    )
    parser.add_argument(
        "--basis",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional metrics plot image path (default: h4_square_error_plot.png).",
    )
    return parser
...
def _plot_metrics(
    ...
    ax1.set_title("H4 Square ADAPT/QSC-EOM Metrics vs ADAPT Iteration")
...
def main():
    ...
    geometry = [
        [0.0, 0.0, 0.0],
        [args.side_length, 0.0, 0.0],
        [0.0, args.side_length, 0.0],
        [args.side_length, args.side_length, 0.0],
    ]
    ...
    reports = ["===== H4 Square Ground-State ADAPT-VQE ====="]
    iterations = []
    adapt_errors = []
    qsceom_errors = []
    adapt_max_gradients = []
    ...
    for adapt_it in args.adapt_it:
        ...
        lines = [
            f"Square side length (Angstrom): {args.side_length}",
            f"Basis: {args.basis}",
    ...
    if args.output_file is None:
        output_path = Path(__file__).resolve().parent / "h4_square_ground_output.txt"
    else:
    ...
    if args.plot_file is None:
        plot_path = output_path.with_name("h4_square_error_plot.png")
    else:
